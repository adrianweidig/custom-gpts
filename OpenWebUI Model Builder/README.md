...
- OpenWebUI-Grundkenntnisse für die Bewertung von Modellparametern, Capabilities und Wissensdateien.
- Zugriff auf eine OpenWebUI-Umgebung, wenn erzeugte Modellpakete praktisch getestet werden sollen.

## Für Repo-Nutzer

Startpunkt ist `customgpt_infos.md`. Für die eigentliche Modell- und Promptlogik danach `systemprompt.md` und `fachwissen.md` lesen. Die Sammlung `Problemfälle/` hilft bei typischen Aufgabenmodellen und Grenzfällen.

## Hinweise

- Technische Modell-IDs und Dateinamen bleiben bewusst slug-fähig und werden nicht eingedeutscht.
- Prüfe erzeugte `model.json`-Beispiele vor dem Import gegen die tatsächlich eingesetzte OpenWebUI-Version.
